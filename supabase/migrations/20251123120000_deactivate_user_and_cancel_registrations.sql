-- Migration: deactivate_user_and_cancel_registrations
-- Data: 2025-11-23

-- Esta função desativa um usuário e cancela suas inscrições em eventos.
-- Quando possível, use a RPC para executar isto com privilégios elevados (security definer).
...
-- Recomendações:
-- - Garanta que esta função seja criada por um usuário com privilégios (ex.: role 'postgres' ou owner apropriado).
-- - Se utilizar RLS, marque a função como SECURITY DEFINER e defina OWNER para uma role com permissões para atualizar as tabelas citadas.
